...
const DEFAULT_NOTES: Note[] = [
  {
    ...
    title: "Strategy",
    tag: "Roadmap",
    body: "Focus on dense local discovery loops and repeat contact actions.",
  ...
  },
  {
    ...
    tag: "Capital",
    body: "Track growth, city density, leads generated, and monetization readiness.",
    updatedAt: new Date().toISOString(),
  },
];
...
export default function FounderVault() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setCounts({
        users: (c.count ?? 0) + (w.count ?? 0) + (s.count ?? 0),
        customers: c.count ?? 0,
        workers: w.count ?? 0,
        shops: s.count ?? 0,
        leads: contactRows.length,
        calls: contactRows.filter((x: { contact_type?: string }) => x.contact_type === "call")
          .length,
        whatsapp: contactRows.filter(
          (x: { contact_type?: string }) => x.contact_type === "whatsapp",
        ).length,
      });
      const cityMap = new Map<string, number>();
      [...(cr.data ?? []), ...(wr.data ?? []), ...(sr.data ?? [])].forEach(
        (x: { area?: string }) => {
          const k = (x.area || "Unknown").trim();
      ...
      [
        ...(wr.data ?? []).map((x: { job_type?: string }) => x.job_type),
        ...(sr.data ?? []).map((x: { category?: string }) => x.category),
      ].forEach((x) => {
        const k = (x || "Uncategorized").trim();
        ...
        <div className="space-y-6 text-slate-100">
          <section className="overflow-hidden rounded-3xl border border-cyan-300/20 bg-slate-950 p-6 shadow-2xl shadow-slate-950/30">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs text-cyan-100">
                  <Crown className="h-3.5 w-3.5" /> Founder-only
                </div>
                <h2 className="mt-4 text-3xl font-black tracking-tight">Necto Founder Vault</h2>
                <p className="mt-2 max-w-2xl text-sm text-slate-400">
                  Private command center for strategy, risk, revenue readiness, growth signals, and
                  system checklist. No secrets or environment variables are displayed.
                </p>
              </div>
              <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 px-4 py-3 text-sm text-emerald-100">
                Access: main founder/admin only
              </div>
            </div>
          </section>
          ...
          <div className="grid gap-4 xl:grid-cols-3">
            ...
            />
            <VaultPanel
              title="Weak areas needing growth"
              icon={ShieldAlert}
              items={weakAreas}
              empty="No weak areas detected"
            />
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <section className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-xl backdrop-blur">
              <div className="mb-4 flex items-center gap-2">
                ...
                <h3 className="font-bold">Secret founder notes</h3>
              </div>
              <div className="grid gap-2 md:grid-cols-3">
                <input
                  ...
                  placeholder="Title"
                  className="rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm outline-none"
                />
                <input
                  value={draft.tag}
              ...
              <textarea
                ...
                placeholder="Strategy, roadmap, competitor observations, investor notes, marketing ideas..."
                className="mt-2 min-h-24 w-full rounded-xl border border-white/10 bg-slate-950/60 px-3 py-2 text-sm outline-none"
              />
                ...
                {notes.map((n) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="rounded-full bg-cyan-300/10 px-2 py-0.5 text-xs text-cyan-100">
                        {n.tag}
                      </span>
                    </div>
            ...
            <section className="rounded-3xl border border-white/10 bg-white/10 p-5 shadow-xl backdrop-blur">
              <div className="mb-4 flex items-center gap-2">
                ...
                <h3 className="font-bold">Private system checklist</h3>
              </div>
              {[
                "SEO status",
                "Analytics status",
                ...
                >
                  <span>{x}</span>
                  <span className={i === 5 ? "text-amber-200" : "text-emerald-200"}>
                    {i === 5 ? "Review" : "Operational"}
                  </span>
                </div>
              ))}
              <div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/10 p-3 text-sm text-amber-100">
                Risk monitor: watch for sudden registration/contact spikes and repeated low-quality
                accounts.
              </div>
            </section>
          </div>
...
function VaultMetric({ icon: Icon, label, value }: { icon: any; label: string; value: number }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-slate-950 p-5 text-white shadow-xl transition hover:-translate-y-1">
...
function VaultPanel({
  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-slate-950 p-5 shadow-xl">
      <div className="mb-4 flex items-center gap-2">
          ...
          {items.map((x) => (
            <div key={x.name}>
              <div className="mb-1 flex justify-between text-sm">
                <span>{x.name}</span>
                <span className="text-cyan-200">{x.value}</span>
              </div>
              <div className="h-2 overflow-hidden rounded-full bg-white/10">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-emerald-300"
                  style={{ width: `${Math.min(100, x.value * 14)}%` }}
                />
              </div>
            </div>
          ))}
