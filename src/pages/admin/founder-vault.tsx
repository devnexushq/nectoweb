  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setCounts({
        ...
        calls: contactRows.filter((x: any) => x.contact_type === "call").length,
        whatsapp: contactRows.filter((x: any) => x.contact_type === "whatsapp").length,
      });
      const cityMap = new Map<string, number>();
      [...(cr.data ?? []), ...(wr.data ?? []), ...(sr.data ?? [])].forEach((x: any) => {
      ...
      [
        ...(wr.data ?? []).map((x: any) => x.job_type),
        ...(sr.data ?? []).map((x: any) => x.category),
      ].forEach((x) => {
