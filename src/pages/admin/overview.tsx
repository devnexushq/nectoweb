  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [
        ...
        supabase
          .from("customers")
          .select("id,name,area,approval_status,created_at")
          .order("created_at", { ascending: false })
          .limit(6),
        supabase
          .from("workers")
          .select("id,name,job_type,area,approval_status,registered_at")
          .order("registered_at", { ascending: false })
          .limit(6),
        supabase
          .from("shops")
          .select("id,shop_name,category,area,approval_status,registered_at")
          .order("registered_at", { ascending: false })
          .limit(6),
      ]);
      ...
      setCounts({
        ...
        calls: contactRows.filter((l: { contact_type?: string }) => l.contact_type === "call")
          .length,
        whatsapp: contactRows.filter(
          (l: { contact_type?: string }) => l.contact_type === "whatsapp",
        ).length,
      });
    ...
    <RequireAdmin>
      <AdminLayout title="Overview">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5 md:gap-4">
          ...
          <StatCard
            label="Workers"
            value={loading ? "—" : counts!.workers}
            icon={Hammer}
            accent="bg-amber-50 text-amber-600"
          />
          <StatCard
            label="Shops"
            value={loading ? "—" : counts!.shops}
            icon={Store}
          ...
          <StatCard
            label="Calls"
            value={loading ? "—" : counts!.calls}
            icon={Phone}
            accent="bg-rose-50 text-rose-600"
          />
          <StatCard
            label="WhatsApp"
            value={loading ? "—" : counts!.whatsapp}
            icon={MessageCircle}
            accent="bg-emerald-50 text-emerald-600"
          />
          <StatCard
            label="Support"
            value={loading ? "—" : counts!.support}
            icon={LifeBuoy}
        ...
        <div className="mt-6 rounded-2xl border border-white/70 bg-white/90 shadow-xl shadow-slate-200/70 backdrop-blur">
          <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
            ...
            <span className="ml-auto text-xs text-slate-500">Customers, workers, and shops</span>
          </div>
          <div className="divide-y divide-slate-100">
            {loading ? (
              ...
              latestRegistrations.map((r) => (
                <div
                  key={`${r.type}-${r.id}`}
                  className="grid gap-2 px-4 py-3 text-sm md:grid-cols-[120px_1fr_160px_130px_150px] md:items-center"
                >
                  <StatusBadge status={r.type} />
                  <div className="min-w-0">
                    <div className="truncate font-semibold text-slate-950">{r.name}</div>
                    <div className="truncate text-xs text-slate-500">{r.category}</div>
                  </div>
        ...
        <div className="mt-6 rounded-2xl border border-white/70 bg-white/90 shadow-xl shadow-slate-200/70 backdrop-blur">
          <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
            ...
            <h2 className="text-sm font-semibold">Recent activity feed</h2>
          </div>
          <div className="divide-y divide-slate-100">
            {recent.length === 0 ? (
