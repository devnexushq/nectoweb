...
export default function AdminAnalytics() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      (wRows.data ?? []).forEach((r: { registered_at: string }) =>
        bump(r.registered_at, "workers"),
      );
      ...
      const needsReview =
        ...
        (rejectedS.count ?? 0);
      const restricted =
        (suspendedC.count ?? 0) + (suspendedW.count ?? 0) + (suspendedS.count ?? 0);
      setAccountHealth([
    ...
    <RequireAdmin>
      <AdminLayout title="Analytics">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
          <StatCard
            label="Customers"
            value={loading ? "-" : counts.c}
            icon={Users}
            accent="bg-indigo-50 text-indigo-600"
          />
          <StatCard
            label="Workers"
            value={loading ? "-" : counts.w}
            icon={Hammer}
          ...
          <StatCard
            label="Products"
            value={loading ? "-" : counts.p}
            icon={Package}
            accent="bg-purple-50 text-purple-600"
          />
          <StatCard
            label="Support"
            value={loading ? "-" : counts.sq}
            icon={LifeBuoy}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
            ...
            <p className="mb-3 text-xs text-slate-500">
              Live account availability and moderation snapshot.
            </p>
            <div className="h-72">
              <ResponsiveContainer>
                <PieChart>
