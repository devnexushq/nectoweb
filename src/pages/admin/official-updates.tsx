...
const CONTROL_CLASS =
  "mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-200";
const ACTION_CLASS =
    ...
    if (result.error) {
      ...
      setMessage(
        statusOverride === "published"
          ...
          : "Official update saved as draft.",
      );
      setForm(EMPTY_FORM);
      await load();
    }
    ...
    <RequireAdmin>
      <AdminLayout title="Official Updates Manager">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-slate-300">
                Publish admin-approved announcements to Customer, Worker, and Shop Activity Centers.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
                <div className="text-2xl font-black">{stats.published}</div>
                <div>Published</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-3">
                <div className="text-2xl font-black">{stats.hidden}</div>
                <div>Hidden</div>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(22rem,0.8fr)_1.2fr]">
          <form
            onSubmit={onSubmit}
            className="rounded-2xl border border-white/70 bg-white/90 p-4 shadow-xl shadow-slate-200/70 backdrop-blur"
          >
            ...
            <div className="space-y-3">
              <Field label="Title">
                <input
                  ...
                  placeholder="Short official title"
                />
              </Field>
              <Field label="Message">
                <textarea
                  ...
                  className={`${CONTROL_CLASS} min-h-36 resize-y`}
                  placeholder="Full official message"
                />
              </Field>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Target role">
                  <select
                    ...
                    onChange={(e) =>
                      updateField("target_role", e.target.value as ActivityTargetRole)
                    }
                    className={CONTROL_CLASS}
                  >
                ...
                <Field label="Visibility">
                  <select
                    ...
                    onChange={(e) =>
                      updateField("visibility_scope", e.target.value as ActivityVisibilityScope)
                    }
                    className={CONTROL_CLASS}
                  >
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <button
                disabled={saving}
                type="submit"
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white font-bold text-slate-900 hover:bg-slate-50 disabled:opacity-60"
              >
              ...
              <button
                ...
                className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 font-bold text-white hover:bg-slate-800 disabled:opacity-60"
              >
          ...
          <section className="rounded-2xl border border-white/70 bg-white/90 shadow-xl shadow-slate-200/70 backdrop-blur">
            <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
              ...
              <h3 className="font-bold">Official updates</h3>
            </div>
                ...
                rows.map((row) => (
                  <article key={row.id} className="p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => changeStatus(row, "hidden")}
                        className={ACTION_CLASS}
                      >
                      ...
                      <button
                        ...
                        className={`${ACTION_CLASS} text-rose-600 hover:border-rose-200 hover:bg-rose-50`}
                      >
