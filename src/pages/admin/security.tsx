      ...
      <AdminLayout title="Security Center">
        <div className="grid gap-4 md:grid-cols-3">
          <SecurityCard
            ...
            status="Active"
            tone="emerald"
            body="Admin pages are guarded by Supabase session and admin role checks."
          />
          <SecurityCard
            icon={Lock}
            ...
            status="Blocked"
            tone="cyan"
            body="No service role keys, JWT secrets, database passwords, or environment values are rendered."
          />
          <SecurityCard
            icon={AlertTriangle}
            title="Risk monitor"
            status="Normal"
            tone="amber"
            body="Review audit logs for unusual admin actions, traffic spikes, and suspicious records."
          />
        </div>
        <section className="mt-6 rounded-2xl border border-white/70 bg-white/90 p-4 shadow-xl shadow-slate-200/70">
          <div className="mb-3 flex items-center gap-2">
            <Activity className="h-4 w-4 text-slate-500" />
            <h2 className="text-sm font-semibold">Recent security-relevant activity</h2>
          </div>
          <div className="divide-y divide-slate-100">
            {loading ? (
              <div className="py-8 text-sm text-slate-500">Loading...</div>
            ) : recent.length === 0 ? (
              <div className="py-8 text-sm text-slate-500">No activity yet</div>
            ) : (
              recent.map((r) => (
...
function SecurityCard({
  ...
  return (
    <div className="rounded-2xl border border-white/70 bg-white/90 p-5 shadow-xl shadow-slate-200/70 transition hover:-translate-y-1">
      <div
        className={`mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-gradient-to-br ${colors[tone]} text-white shadow-lg`}
      >
